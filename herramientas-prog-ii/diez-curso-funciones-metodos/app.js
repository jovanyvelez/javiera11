/* ============================================================
   CLASE diez — HERRAMIENTAS DE PROGRAMACIÓN II
   Funciones y métodos (def, parámetros, retorno, reutilización)
...
const STORAGE_KEY = 'curso-herr-10-funciones';
...
function marcarCompletado(n) {
  ...
  const badgesModulo = {0: "🚀 Iniciado", 1: "📦 def", 2: "🔌 Parámetros", 3: "📤 Retorno", 4: "☕ Descansado", 5: "♻️ Reutiliza", 6: "🧬 Métodos", 7: "🛠️ Taller Funciones"};
  if (badgesModulo[n]) otorgarBadge(badgesModulo[n]);

  if (estado.completados.size === TOTAL_MODULOS) {
    otorgarBadge('🏆 Funciones Dominadas');
  }
...
function actualizarUI() {
  ...
  if (mAct) {
    const labels = ['Inicio', 'def', 'Parámetros', 'Retorno', 'Descanso', 'Reutilizar', 'Métodos', 'Taller'];
