/* ============================================================
   CLASE cinco — HERRAMIENTAS DE PROGRAMACIÓN II
...
const STORAGE_KEY = 'curso-herr-5';
...
function marcarCompletado(n) {
  ...
  const badgesModulo = {0: "🚀 Iniciado", 1: "🧱 Modelos", 2: "🏷️ Field", 3: "📝 Form", 4: "☕ Descansado", 5: "🚫 422", 6: "📐 Esquemas", 7: "🛠️ Pydantic Master"};
  if (badgesModulo[n]) otorgarBadge(badgesModulo[n]);

  if (estado.completados.size === TOTAL_MODULOS) {
    otorgarBadge('🏆 Pydantic Dominado');
  }
...
function actualizarUI() {
  ...
  if (mAct) {
    const labels = ['Inicio', 'Modelos', 'Field', 'Form', 'Descanso', '422', 'Esquemas', 'Taller'];
