/* ============================================================
   CLASE siete — HERRAMIENTAS DE PROGRAMACIÓN II
...
const STORAGE_KEY = 'curso-herr-7';
...
function marcarCompletado(n) {
  ...
  const badgesModulo = {0: "🚀 Iniciado", 1: "🎨 Bootstrap", 2: "⚡ htmx", 3: "📤 hx-post", 4: "☕ Descansado", 5: "🎯 hx-target", 6: "🔁 hx-swap", 7: "🛠️ htmx Master"};
  if (badgesModulo[n]) otorgarBadge(badgesModulo[n]);

  if (estado.completados.size === TOTAL_MODULOS) {
    otorgarBadge('🏆 htmx + Bootstrap Dominado');
  }
...
function actualizarUI() {
  ...
  if (mAct) {
    const labels = ['Inicio', 'Bootstrap', 'htmx', 'hx-post', 'Descanso', 'hx-target', 'hx-swap', 'Taller'];
