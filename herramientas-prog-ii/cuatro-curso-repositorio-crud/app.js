/* ============================================================
   CLASE cuatro — HERRAMIENTAS DE PROGRAMACIÓN II
...
const STORAGE_KEY = 'curso-herr-4';
...
function marcarCompletado(n) {
  ...
  const badgesModulo = {0: "🚀 Iniciado", 1: "📚 Repositorio", 2: "🔍 SELECT", 3: "➕ INSERT", 4: "☕ Descansado", 5: "✏️ UPDATE", 6: "🗑️ DELETE", 7: "🛠️ CRUD Master"};
  if (badgesModulo[n]) otorgarBadge(badgesModulo[n]);

  if (estado.completados.size === TOTAL_MODULOS) {
    otorgarBadge('🏆 Repositorio CRUD Dominado');
  }
...
function actualizarUI() {
  ...
  if (mAct) {
    const labels = ['Inicio', 'Repositorio', 'SELECT', 'INSERT', 'Descanso', 'UPDATE', 'DELETE', 'Taller'];
