/* ============================================================
   CLASE dos — HERRAMIENTAS DE PROGRAMACIÓN II
...
const STORAGE_KEY = 'curso-herr-2';
...
function marcarCompletado(n) {
  ...
  const badgesModulo = {0: "🚀 Iniciado", 1: "🧩 Framework", 2: "🛣️ Rutas", 3: "🔄 Lifespan", 4: "☕ Descansado", 5: "🔗 Depends", 6: "📚 Docs", 7: "🛠️ FastAPI Master"};
  if (badgesModulo[n]) otorgarBadge(badgesModulo[n]);

  if (estado.completados.size === TOTAL_MODULOS) {
    otorgarBadge('🏆 FastAPI Básico Dominado');
  }
...
function actualizarUI() {
  ...
  if (mAct) {
    const labels = ['Inicio', 'Framework', 'Rutas', 'Lifespan', 'Descanso', 'Depends', 'Docs', 'Taller'];
