/* ============================================================
   CLASE tres — HERRAMIENTAS DE PROGRAMACIÓN II
...
const STORAGE_KEY = 'curso-herr-3';
...
function marcarCompletado(n) {
  ...
  const badgesModulo = {0: "🚀 Iniciado", 1: "🐘 PostgreSQL", 2: "⚡ asyncpg", 3: "🏊 Pool", 4: "☕ Descansado", 5: "🔗 get_connection", 6: "🔄 Lifespan", 7: "🛠️ BD Master"};
  if (badgesModulo[n]) otorgarBadge(badgesModulo[n]);

  if (estado.completados.size === TOTAL_MODULOS) {
    otorgarBadge('🏆 BD + Pool Dominado');
  }
...
function actualizarUI() {
  ...
  if (mAct) {
    const labels = ['Inicio', 'PostgreSQL', 'asyncpg', 'Pool', 'Descanso', 'get_connection', 'Lifespan', 'Taller'];
